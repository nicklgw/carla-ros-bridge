...
shenlan::control::PIDController speed_pid_controller(1.0, 0.3, 0.0);    // 速度pids
...
**************************************************************************************'''*/
{
    ...
    vehicle_control_publisher = this->create_publisher<carla_msgs::msg::CarlaEgoVehicleControl>("/carla/ego_vehicle/vehicle_control_cmd_manual", 10);
    ...
    for (size_t i = 0; i < headings.size(); i++) {
        ...
        if (i < _stop_begin_point) {
            ...
            if (trajectory_pt.v > 1.0) {
                trajectory_pt.v = 10.0 * ((double)i / ((double)_stop_begin_point - (double)_stop_point) - (double)_stop_point / ((double)_stop_begin_point - (double)_stop_point));
            } else {
                trajectory_pt.v = 0;
            }
        }
